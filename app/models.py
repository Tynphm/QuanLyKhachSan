...
class TrangThaiDonDatPhong(UserEnum):
    CHUA_THANH_TOAN = 1
    DA_THANH_TOAN = 2
...
class Phong(db.Model):
    ...
    TrangThai = db.Column(db.String(50), default='available')  # Trạng thái phòng (available, paid)
...
class DonDatPhong(db.Model):
    ...
    MaLoaiPhong = Column(Integer, ForeignKey(LoaiPhong.MaLoaiPhong))
    ...
    HoTenNguoiDat = Column(String(30), nullable=False)
    NgayNhanPhong = Column(Date, nullable=True)
    NgayTraPhong = Column(Date, nullable=True)
    Phone = Column(String(20), nullable=False)
    TaiKhoan = Column(Integer, ForeignKey(TaiKhoan.MaTK), nullable=True)
    trang_thai = Column(Enum(TrangThaiDonDatPhong), default=TrangThaiDonDatPhong.CHUA_THANH_TOAN)
...
class ChiTietDonDatPhong(db.Model):
    ...
    MaLoaiPhong = Column(Integer, ForeignKey(LoaiPhong.MaLoaiPhong))
    MaDonDatPhong = Column(Integer, ForeignKey(DonDatPhong.MaDonDatPhong))
    HoTenNguoiDat = Column(String(30), nullable=False)
    Phone = Column(String(20), nullable=False)
...
class HoaDon(db.Model):
    ...
    DonDatPhong = Column(Integer, ForeignKey(DonDatPhong.MaDonDatPhong), nullable=False)
...
if __name__ == '__main__':
    with app.app_context():
        ...
        taiKhoan = read_json('data/TaiKhoan.json')
        for i in taiKhoan:
            a = TaiKhoan(TenTK=i["TenTK"], Username=i["Username"], Password=str(hashlib.md5(i["Password"].encode('utf-8')).hexdigest()), Email=i["Email"], Phone=i["Phone"], user_role=i["UserRole"])
            db.session.add(a)
            db.session.commit()
